...
VENV_PIW = "pythonw.exe" if IS_WIN else "python3"   # pythonw doesn't exist on Linux
...
class GitHubAPI:
    def __init__(self, token=""):
        ...
        if token:
            # GitHub accepts Bearer for PATs (modern form); avoids deprecated token prefix in new tooling.
...
class Installer:
    ...
    def _release(self, owner, repo, d, aid):
        ...
        if not asset: self.log("  ✗ No suitable release asset found for this platform."); return False
        dest=d/asset["name"]
    ...
    def _node(self, owner, repo, d, aid):
        ...
        if not shutil.which("npm"):
            self.log("  ✗ npm not found. Install Node.js (npm ships with it) and ensure it is on PATH.")
            return False
        if not (d/".git").exists():
            r = subprocess.run(
                git_clone_args(owner, repo, str(d), self.api.token),
...
class App(tk.Tk):
    ...
    def _open_build(self):
        ...
        if IS_WIN:
            ...
            desc_txt=("Uses PyInstaller to compile this app into a single .exe file.\n"
                      "The result runs on any Windows PC — no Python needed.")
        ...
        def _build():
            ...
            subprocess.run([sys.executable,"-m","pip","install","pyinstaller","-q"],capture_output=True)
            _w("Running PyInstaller (this takes a few minutes)...")
            r=subprocess.run([
                sys.executable,"-m","PyInstaller",
                "--onefile","--windowed" if IS_WIN else "--onefile",
                "--clean","--name=GitHubAppManager",
                f"--distpath={out_dir}",f"--workpath={APP_DIR/'_build'}",
                f"--specpath={APP_DIR}",str(script)
    ...
    def _open_settings(self):
        win=tk.Toplevel(self); win.title("Settings"); win.geometry("560x300")
        ...
        tk.Label(win,
                 text="Required to access private repos and raise the API rate limit.\n\n"
                      "How to create one:\n"
                      "  github.com → Settings → Developer settings\n"
                      "  → Personal access tokens → Tokens (classic)\n"
                      "  → Generate new token → ✓ check 'repo' → Generate → Copy",
        ...
        def _save():
            self.registry.set_token(tv.get().strip()); self._rebuild_api()
            win.destroy(); self._check_token_status()
            self._log("  ✓ Token saved. Go to 👤 My Profile → Load / Refresh.")
        styled_btn(win,"Save Token",_save,primary=True).pack(pady=8)
